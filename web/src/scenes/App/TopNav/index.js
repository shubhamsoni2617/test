...
const TopNav = props => {
  ...
  const [isActiveIndex, setIsActiveIndex] = useState("random no");
                ...
                <ul>
                  {byGenreEvent &&
                    byGenreEvent
                      ...
                      .map((event, index, arr) => {
                        return (
                          <li key={event.id} className={isActiveIndex === index ? "active" : ""} onClick={() => { setIsActiveIndex(index) }}>
